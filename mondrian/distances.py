...
def custom_distance(X, Y, file_area, alpha=1, beta=0, gamma=0.1):
    ...
    # to make sure ratio is always smaller/bigger
...
def rectangle_as_array(obj):
    """To be used in order to parallelize the distance measure"""
    x_0 = obj["top_lx"][0]
    x_1 = obj["top_lx"][1]
    x_2 = obj["bot_rx"][0]
    x_3 = obj["bot_rx"][1]
...
def parallel_distance(X, Y, file_width, file_height, alpha, beta, gamma, lbp=False, print_metric=False):
    ...
    geo_dist = dist_closest(X, Y) * is_connected(X, Y)
    ...
    h_alignment = (np.abs(X[2] - Y[2]) + np.abs(X[0] - Y[0])) / max(X[6], Y[6])
    v_alignment = (np.abs(X[3] - Y[3]) + np.abs(X[1] - Y[1])) / max(X[7], Y[7])
